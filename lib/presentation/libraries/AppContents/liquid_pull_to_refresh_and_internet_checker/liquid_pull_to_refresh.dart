library liquid_pull_to_refresh_and_internet_checker;
...
/// finished.
///
/// Used by [LiquidPullToRefresh.onRefreshComplete].
...
enum _LiquidPullToRefreshMode {
  ...
  armed, // Dragged far enough that an up event will run the onRefreshComplete callback.
...
class LiquidPullToRefresh extends StatefulWidget {
  ...
  /// The progress indicator's foreground backgroundColor. The current theme's
  /// [ThemeData.colorScheme.secondary] by default.
  final Color? backgroundColor;

  /// The progress indicator's background backgroundColor. The current theme's
...
class LiquidPullToRefreshState extends State<LiquidPullToRefresh>
  ...
  void didChangeDependencies() {
    ...
    _valueColor = _positionController.drive(
      ColorTween(
          begin: (widget.backgroundColor ?? theme.colorScheme.secondary).withOpacity(0.0),
          end: (widget.backgroundColor ?? theme.colorScheme.secondary).withOpacity(1.0))
          .chain(CurveTween(
  ...
  void _show() {
    ...
    _springController
      ...
      if (mounted && _mode == _LiquidPullToRefreshMode.snap) {
        ...
        final Future<void> refreshResult = handleRefresh();
        assert(() {
          if (refreshResult == null) {
            ...
            FlutterError.reportError(FlutterErrorDetails(
              exception: FlutterError('The onRefreshComplete callback returned null.\n'
                  'The LiquidPullToRefresh onRefreshComplete callback must return a Future.'),
              context: safeContext('when calling onRefreshComplete'),
  ...
  /// [LiquidPullToRefresh.onRefreshComplete] callback's future completes.
  ///
  ...
  Widget build(BuildContext context) {
    ...
    // assigning default backgroundColor and background backgroundColor
    ...
    Color backgroundColor = (widget.backgroundColor != null) ? widget.backgroundColor! : _defaultColor;
    Color circleColor = (widget.circleColor != null)
        ? widget.circleColor!
